...
import wd.android.util.applications.InterestingConfigChanges;
...
public class ApplicationsState {
    ...
    /**
     * Value for {@link #flags}: true if the application is currently installed
    ...
    public static final Comparator<AppEntry> ALPHA_COMPARATOR = new Comparator<AppEntry>() {
        ...
        public int compare(AppEntry object1, AppEntry object2) {
            final boolean normal1 = object1.info.enabled
                    && (object1.info.flags & FLAG_INSTALLED) != 0;
            final boolean normal2 = object2.info.enabled
                    && (object2.info.flags & FLAG_INSTALLED) != 0;
            if (normal1 != normal2) {
                return normal1 ? -1 : 1;
            }
    ...
    // final CanBeOnSdCardChecker mCanBeOnSdCardChecker = new
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MSG_PACKAGE_SIZE_CHANGED: {
                    for (int i = 0; i < mActiveSessions.size(); i++) {
                        mActiveSessions.get(i).mCallbacks
                                .onPackageSizeChanged((String) msg.obj);
                    }
                }
                break;
                ...
                case MSG_RUNNING_STATE_CHANGED: {
                    for (int i = 0; i < mActiveSessions.size(); i++) {
                        mActiveSessions.get(i).mCallbacks
                                .onRunningStateChanged(msg.arg1 != 0);
                    }
                }
                break;
            }
    ...
    private ApplicationsState(Application app) {
         ...
         * contention on it. It doesn't figure this out fast enough for us here,
         * though, so this little trick will force it to turn into a real lock
         * immediately.
         */
        synchronized (mEntriesMap) {
            try {
    ...
    public class Session {
            ...
            synchronized (mEntriesMap) {
                if (mResumed) {
                    ...
                    mBackgroundHandler.removeMessages(
                            BackgroundHandler.MSG_REBUILD_LIST, this);
        ...
        // Creates a new list of app entries with the given filter and
        // comparator.
        ArrayList<AppEntry> rebuild(AppFilter filter,
            ...
            synchronized (mRebuildSync) {
                synchronized (mEntriesMap) {
                    ...
                    mRebuildResult = null;
                    if (!mBackgroundHandler
                            .hasMessages(BackgroundHandler.MSG_REBUILD_LIST)) {
                        Message msg = mBackgroundHandler
                                .obtainMessage(BackgroundHandler.MSG_REBUILD_LIST);
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                if (filter == null || filter.filterApp(info)) {
                    synchronized (mEntriesMap) {
                        ...
                        if (DEBUG)
                            Log.i(TAG, "Using " + info.packageName + ": "
                                    + entry);
    ...
    void doResumeIfNeededLocked() {
        ...
        for (int i = 0; i < mApplications.size(); i++) {
            ...
            int enabledSetting = mContext.getPackageManager()
    ...
    void requestSize(String packageName) {
        ...
        synchronized (mEntriesMap) {
            ...
            if (entry != null) {
                // TODO gggg
                // mPm.getPackageSizeInfo(packageName,
    ...
    void addPackage(String pkgName) {
        try {
            synchronized (mEntriesMap) {
                ...
                if (!mBackgroundHandler
                        .hasMessages(BackgroundHandler.MSG_LOAD_ENTRIES)) {
                    mBackgroundHandler
                            .sendEmptyMessage(BackgroundHandler.MSG_LOAD_ENTRIES);
                }
                if (!mMainHandler
                        .hasMessages(MainHandler.MSG_PACKAGE_LIST_CHANGED)) {
                    mMainHandler
                            .sendEmptyMessage(MainHandler.MSG_PACKAGE_LIST_CHANGED);
                }
                if (DEBUG_LOCKING)
    ...
    AppEntry getEntryLocked(ApplicationInfo info) {
        ...
        if (DEBUG)
            Log.i(TAG, "Looking up entry of pkg " + info.packageName + ": "
                    + entry);
        if (entry == null) {
            if (DEBUG)
    ...
    class BackgroundHandler extends Handler {
        ...
        // TODO ggg
        // final IPackageStatsObserver.Stub mStatsObserver = new
        ...
        // synchronized (mEntriesMap) {
        // if (DEBUG_LOCKING)
        // Log.v(TAG, "onGetStatsCompleted acquired lock");
        ...
        // if (DEBUG)
        // Log.i(TAG, "Set size of " + entry.label
        // + " " + entry + ": "
        // + entry.sizeStr);
        // sizeChanged = true;
        // }
        ...
        // if (sizeChanged) {
        // Message msg = mMainHandler.obtainMessage(
        // MainHandler.MSG_PACKAGE_SIZE_CHANGED,
        // stats.packageName);
        // mMainHandler.sendMessage(msg);
        // }
        // }
        ...
        // }
        // if (DEBUG_LOCKING)
        // Log.v(TAG, "onGetStatsCompleted releasing lock");
        // }
        // }
        // };
        ...
        public void handleMessage(Message msg) {
            ...
            synchronized (mEntriesMap) {
                if (mRebuildingSessions.size() > 0) {
                    rebuildingSessions = new ArrayList<Session>(
                            mRebuildingSessions);
            ...
            switch (msg.what) {
                ...
                case MSG_LOAD_ENTRIES: {
                    ...
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_ENTRIES acquired lock");
                        ...
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_ENTRIES releasing lock");
                    }
                ...
                case MSG_LOAD_ICONS: {
                    ...
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_ICONS acquired lock");
                        ...
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_ICONS releasing lock");
                    }
                    if (numDone > 0) {
                        if (!mMainHandler
                                .hasMessages(MainHandler.MSG_PACKAGE_ICON_CHANGED)) {
                            mMainHandler
                                    .sendEmptyMessage(MainHandler.MSG_PACKAGE_ICON_CHANGED);
                        }
                    }
                    if (numDone >= 2) {
                        sendEmptyMessage(MSG_LOAD_ICONS);
                ...
                case MSG_LOAD_SIZES: {
                    synchronized (mEntriesMap) {
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_SIZES acquired lock");
                        if (mCurComputingSizePkg != null) {
                            if (DEBUG_LOCKING)
                                Log.v(TAG,
                        ...
                        if (!mMainHandler
                                .hasMessages(MainHandler.MSG_ALL_SIZES_COMPUTED)) {
                            mMainHandler
                                    .sendEmptyMessage(MainHandler.MSG_ALL_SIZES_COMPUTED);
                            mRunning = false;
                            Message m = mMainHandler.obtainMessage(
                        ...
                        if (DEBUG_LOCKING)
                            Log.v(TAG, "MSG_LOAD_SIZES releasing lock");
                    }
                }
                break;
            }
